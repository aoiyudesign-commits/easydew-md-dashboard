...
- ✅ **VOC 모니터링 피드**(② 탭 상단): 특정 제품에 한정하지 않고 리뷰 게시판 전체 카테고리에서 수집한 실제 리뷰 36건을 "신규 리뷰 인박스" 형태로 표시. `classifyCategory()`/`classifyIssues()`가 제품명·리뷰 원문을 규칙 기반으로 자동 분류(카테고리 태그·이슈 태그)하고, 3점 이하는 "CS 대응 필요"로 자동 플래그. 카테고리/별점 필터 제공. 실 서비스에서는 스케줄러가 신규 게시물만 수집해 이 배열에 append하면 됨
...
- VOC 모니터링 피드에 "미답변 리뷰 큐"(관리자 답글 여부 추적)와 일별 리뷰량·평균 별점 추이 차트 추가
